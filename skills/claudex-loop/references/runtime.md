...
For an explicit model choice, add e.g. `--model gpt-6-astra --effort high` to a Codex call, or `--model claude-fable-5-1` to a Claude call. Codex reviews and inspections ignore user configuration, so omitted model/effort values use the CLI's built-in defaults. Builds and Claude calls retain their normal configuration. Repeat explicit model/effort choices when resuming; the runner refuses mismatches. No global configuration is changed.
...
- Codex: `exec -s read-only`; resume uses `-c sandbox_mode="read-only"`. Fresh and resumed reviews and inspections also pass `--ignore-user-config`, so `config.toml` (MCP servers, plugins, hooks, default model) is not loaded; authentication still comes from `CODEX_HOME`. They also set `web_search="disabled"`, which removes the hosted web tool (verified 2026-09-07 with Codex 0.153.4: the isolated reviewer reported no MCP servers and no web tool). This is deliberate: the shell sandbox is not a restriction on external MCP side effects, and a reviewer must not inherit a write-capable toolchain. Because the configured default model is skipped too, pass `--model` and `--effort` explicitly when a specific Codex model is wanted for review. Builds keep normal configuration. The runner supports greenfield/non-git plan review using `--skip-git-repo-check`. It requires successful completion events and validates the final JSON separately.
...
Live-tested development baseline: Codex CLI **0.153.4** with **GPT-6 Astra**, and Claude Code **2.1.261** with **Fable 5.1**, on Windows (last live-verified 2026-09-07). Flag presence only, without a live model call, was last rechecked on 2026-09-16 on macOS against Claude Code 2.1.273 and Codex CLI 0.154.0: all review/build flags above are still listed in `--help`. Update these dates when re-verifying. The older npm Codex CLI 0.144.5 exposed the required flags but Astra rejected it with “requires a newer version of Codex.” A version/help probe alone does not establish model compatibility. Verify the selected binary and account; see the repository's validation record for actual live coverage. The automated suite uses fake CLI processes and does not consume model quota. Optional live smoke tests should use disposable fixtures and an explicit model, never a production build.
