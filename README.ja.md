...
モデルは設定可能です。明示的に選択し、アカウントで利用できる場合は Claude Fable 5.1 や GPT-6 Astra を使えます。Codex の計画レビューと最終検査はユーザー設定を読み込みません。特定のモデルと推論強度を使う場合は明示的に指定してください。省略時は CLI の組み込み既定値を使います。実装と Claude の呼び出しでは通常の設定を維持します。ホスト画面で選んだモデルが別の CLI に自動適用されるわけではありません。要求したモデルと観測できたモデルは別々に記録し、失敗時に黙って切り替えません。
...
| `reviewer_model` / `builder_model` / `inspector_model` | CLI の既定値（上記モデル説明参照） | 役割別モデル指定 |
| `reviewer_effort` / `builder_effort` / `inspector_effort` | CLI の既定値（上記モデル説明参照） | 対応する推論強度の指定 |
...
承認は計画の絶対パスと SHA256 に結び付き、計画を変更すると無効になります。最終検査には実装前のコミットと、ステージ済み・未追跡ファイルを含む変更全体の指紋も記録します。検査中または検査後に変更があれば、再検査が必要です。ステージ済みの変更が作業ツリーの内容と異なる場合、検査は開始できません。意図した内容をステージするか、その変更をステージから外してください。無関係な作業をステージする必要はありません。通常の未ステージ編集は引き続き扱えます。検査後のステージ操作も指紋を変えるため、再検査が必要です。

Codex のレビューは読み取り専用 shell サンドボックスを使い、Git リポジトリでないディレクトリの計画もレビューできます。ユーザーの `config.toml` を読み込まず、Web 検索も無効にするため、ユーザー設定の MCP ツールはレビュー担当に渡りません。Claude のレビューはファイルの読み取りと検索のみを許可し、カスタマイズと MCP を無効にします。両者の境界は異なります。委任実装は限定した権限とクリーンな Git 基準状態を使用します。worktree は差分を隔離する仕組みであり、それ自体がセキュリティサンドボックスではありません。
