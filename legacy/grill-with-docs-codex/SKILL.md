...
description: "SUPERSEDED by /claudex-loop (formerly /crucible) — prefer that skill unless explicitly wanted. Two-act plan hardening with living documentation. ACT 1 (you ↔ Claude): Claude interviews you one question at a time, challenging the plan against the project's glossary (CONTEXT.md) and ADRs, sharpening fuzzy terms, stress-testing with concrete scenarios, cross-referencing code, and updating CONTEXT.md and ADRs inline. ACT 2 (Claude ↔ Codex): Claude writes the locked plan to PLAN.md, Codex adversarially reviews it read-only (VERDICT:APPROVED/REVISE), and Claude revises in the SAME Codex session until APPROVED or MAX_ROUNDS; you sign off before any code. Use when the user says \"/grill-with-docs-codex\", \"grill me against the docs then have codex review\", \"stress-test this against our domain model then get a second model on it\", or is building something high-stakes in a project with established terminology/ADRs. Builds on Matt Pocock's grill-with-docs (MIT). NOT for reviewing already-written code or trivial changes."
---
